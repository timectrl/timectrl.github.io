...
linux${EFI} /vmlinuz ip=dhcp inst.repo=${MIRROR_PREFIX}/Everything/x86_64/os inst.ks=hd:UUID=${ROOT_UUID}:/kickstart.txt inst.text inst.vnc inst.vncpassword=PaFfW0rd
#linux${EFI} /vmlinuz ip=dhcp inst.repo=hd:UUID=${ROOT_UUID}:/ inst.ks=hd:UUID=${ROOT_UUID}:/kickstart.txt inst.text inst.vnc inst.vncpassword=PaFfW0rd
